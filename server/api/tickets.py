...
@ticket_bp.route("/<ticket_id>/events", methods=["GET"])
@jwt_required()
@swag_from(os.path.join(_SWAGGER_DIR, "tickets", "list_ticket_events.yml"))
def list_ticket_events(ticket_id):
    ...
    events = (
        TicketEvent.query.filter_by(ticket_id=ticket.id)
        .order_by(TicketEvent.created_at.asc())
        .all()
    )

    hide_internal = current_user.role != "admin" and not (
    ...
    for event in events:
        if (
            hide_internal
            and event.event_type == "comment_added"
            and event.meta
            and event.meta.get("is_internal")
        ):
            continue
        results.append(
            {
